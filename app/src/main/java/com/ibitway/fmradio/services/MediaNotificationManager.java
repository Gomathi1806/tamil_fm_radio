package com.ibitway.fmradio.services;
...
public class MediaNotificationManager {
    ...
    public MediaNotificationManager(RadioService service, Resources resources) {
        this.service = service;
    ...
    public void startNotify(Bitmap notifyIcon, Metadata meta,boolean resume) {
        this.notifyIcon = notifyIcon;
        this.meta = meta;
        startNotify();
    }
    ...
    private void startNotify() {
        ...
        if (notifyIcon == null)
            notifyIcon = BitmapFactory.decodeResource(resources, R.drawable.logo);
        ...
        PendingIntent action = PendingIntent.getService(service, 1, playbackAction, pendingIntentFlags());

        if (PlaybackStatus.PAUSED.equals(playbackStatus)) {
            icon = R.drawable.ic_play_white;
            playbackAction.setAction(RadioService.ACTION_PLAY);
            action = PendingIntent.getService(service, 2, playbackAction, pendingIntentFlags());
        }

        Intent stopIntent = new Intent(service, RadioService.class);
        stopIntent.setAction(RadioService.ACTION_STOP);
        PendingIntent stopAction = PendingIntent.getService(service, 3, stopIntent, pendingIntentFlags());

        Intent rawr = new Intent(service, RadioService.class);
        rawr.setAction(RadioService.ACTION_RESUME);
        PendingIntent pendingIntent = PendingIntent.getService(service, 1, rawr, pendingIntentFlags());
...
//        PendingIntent pendingIntent = PendingIntent.getActivity(service, 0, intent, 0);
        ...
        String title = meta != null && meta.getSong() != null ?
                meta.getSong() : strAppName;
        String subTitle = meta != null && meta.getArtist() != null ?
